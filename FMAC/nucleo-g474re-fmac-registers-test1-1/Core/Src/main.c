...
const int16_t aFIRInputX_q15[INPUT_SIZE_N] = {0, 0, 0, 0x7FFF, 0, 0, 0, 0, 0, 0, 0, 0}; // Dirac impulse
...
//	}
	...
	FMAC->CR |= 2;  // enable WIEN interrupts
	FMAC->WDATA = aFIRInputX_q15[10];  // write x[9]
	FMAC->CR |= 2;  // enable WIEN interrupts
	...
	aX1_expected_no_of_samples[n] = 8;
	aY_expected_no_of_samples[n++] = 2; // stall
	...
	FMAC->WDATA = aFIRInputX_q15[10]; // write x[10]
	FMAC->CR |= 2;  // enable WIEN interrupts
	asm("NOP");
	asm("NOP");
	asm("NOP");
	...
	FMAC->WDATA = aFIRInputX_q15[11]; // write x[11]
	FMAC->CR |= 2;  // enable WIEN interrupts
	asm("NOP");
	asm("NOP");
	asm("NOP");
